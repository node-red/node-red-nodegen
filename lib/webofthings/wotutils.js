...
const addFormatsDraft2019 = require('ajv-formats-draft2019');
...
function normalizeTd(td) {
    ...
    for (const p in td.properties) {
        ...
        if (pdef.hasOwnProperty("security") && typeof pdef.security === 'string') {
            pdef.security = [pdef.security];
        }
        if (pdef.forms) {
    ...
    for (const a in td.actions) {
        ...
        if (adef.hasOwnProperty("security") && typeof adef.security === 'string') {
            adef.security = [adef.security];
        }
        if (adef.forms) {
    ...
    for (const e in td.events) {
        ...
        if (edef.hasOwnProperty("security") && typeof edef.security === 'string') {
            edef.security = [edef.security];
        }
        if (edef.forms) {
    ...
    for (const p in td.properties) {
        ...
        if (forms) {
            forms = forms.filter((f) => (f.hasOwnProperty("href") &&
                                         (f.href.match(/^https?:/) || f.href.match(/^wss?:/) || f.href.match(/^coaps?:/) || f.href.match(/^mqtt:/))));
        }
