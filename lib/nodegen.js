...
function function2node(data, options) {
    return when.promise(function (resolve, reject) {
        ...
        if (data.name === 'function') {
            ...
            var languages = fs.readdirSync(path.join(__dirname, '../templates/function/locales'));
            languages.forEach(function (language) {
                var languageFileTemplate = fs.readFileSync(path.join(__dirname, '../templates/function/locales', language, 'node.json.mustache'), 'utf-8');
                var languageFileSourceCode = mustache.render(languageFileTemplate, params);
