...
function wottd2node(data, options) {
    return when.promise(function (resolve, reject) {
        ...
        // if name is not specified, use td.name for module name.
        if (!data.name || data.name === '') {
            // filtering out special characters
            data.name = 'wot' + td.name.replace(/[^A-Za-z0-9]/g, '').toLowerCase();
        }
        ...
        for (const p in td.properties) {  // convert to array
            if (td.properties.hasOwnProperty(p)) {
                ...
                rwo[p] = {};
                rwo[p].readable = true;
                rwo[p].writable = q.writable;
                rwo[p].observable = q.observable;
            }
        }
        data.actions = [];
        ...
        data.ufName = td.name.replace(/[^A-Za-z0-9]/g, ' ').trim();
        ...
        const nodeSourceCode = mustache.render(nodeTemplate, data);
        fs.writeFileSync(path.join(data.dst, data.module, 'node.js'), nodeSourceCode);
